    ...
    <div class="menu_box">
      <div class="formBox-top">
        <el-form :model="formItem" inline class="clearFix" ref="formItem" label-width="110px">
          <el-form-item label="车站名称：" class="marBottom7">
            <el-input  clearable class="width200" v-model="formItem.userName"/>
          </el-form-item>
          <br>
<!--          <el-button type="primary" v-if="limitBtn.create" @click="addChildrenCode">新增</el-button>-->
...
<!--          </el-table-column>-->
    ...
    <el-dialog
      ...
      <el-form :model="addEdit" ref="addEdit" label-width="120px">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item class="star" label="新增车站ID：">
              <el-input v-model="addEdit.newlyNameId"/>
            </el-form-item>
            <el-form-item class="star" label="新增车站编码：">
...
<!--        </el-form-item>-->
    ...
    <el-dialog
      ...
      <div slot="footer">
        <el-button type="primary" @click="resetCode">取消</el-button>
        <el-button type="primary" v-btn-control @click="synchronous">确定</el-button>
      </div>
    </el-dialog>
  </Box>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      screen(row) {
        this.expands = [];
        row.stationList.forEach(item => {
          if (item.LOCATION_NUMBER !== row.LINE_LOCATION_NUMBER) {
      ...
      getData() {
        var _t = this;
        var location = _t.formItem.userId ? _t.formItem.userId : _t.cookieMap.location_number
        var params = {
          location_type_name: _t.formItem.userName,
      ...
      changeLimit() {
        ...
        var params = {};
        _t.$api.post('/tsGateWay/systemManager/synchronizationStation', params, function (res) {
          if (res.rtCode && res.rtCode === '18014') {
      ...
      synchronous() {
        ...
        if (_t.checkList.length !== 0) {
          _t.$api.post('/tsGateWay/systemManager/addAndDeleteStation',_t.checkList, function (res) {
      ...
      editCode(row) {
        ...
        _t.addEdit.newlyLocation = row.location_number; // 车站位置
    ...
    created() {
      ...
      if (limitMenuList !== null && limitMenuList.length !== 0) {
        ...
        limitBtnArr.forEach((item) => {
          // 查询
          if (item.MENU_ID === "MENU20191012163528yivCZsfJzQbXpW") {
            _t.limitBtn.query = true;
          }
          // 同步
          if (item.MENU_ID === "MENU20191029164248XrVmaUKgLHeQUJ") {
            _t.limitBtn.changeLimit = true;
          }
          // 新增
          if (item.MENU_ID === "MENU20191012163630NwcOzprlcYDXKe") {
            _t.limitBtn.create = true;
          }
          // 修改
          if (item.MENU_ID === "MENU20191012163701IKKxiZhXYGJOFx") {
            _t.limitBtn.delete = true;
          }
          // 详细
          if (item.MENU_ID === "MENU20191012163607QwDqADotjqNiRz") {
