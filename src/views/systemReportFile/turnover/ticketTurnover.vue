    ...
    methods: {
      ...
      getData() {
        ...
        _t.$refs.formData.validate(val => {
          if (val) {
            _t.getHeight();
            if (_t.formItem.lines !== '' && _t.formItem.dateTime.length !== 0 && _t.formItem.interval !== '') {
              // 嵌入网页是否显示
              _t.isShow = true;
              ...
              _t.formIframeSrc = _t.$apiForms +'/tbIetp-reportForm/ReportEmitter?rpt=/4102票种进出站量分时段统计表.brt&params=p_line_id=' + _t.formItem.lines.substr(0, 2) + ';p_interval=' + _t.formItem.interval + ';p_start=' + stopData + ';&grabheight=true&fitwidth=true';
