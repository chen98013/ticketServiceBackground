      ...
      getData() {
        ...
        _t.$refs.formData.validate(val => {
          if (val) {
            ...
            if (_t.formItem.lines !== '' && time && _t.formItem.interval !== '') {
              // 嵌入网页是否显示
              _t.isShow = true;
              var data1 = new Date(_t.formItem.dateTime);

              var stopData = `${data1.getFullYear()}${_t.time(data1.getMonth() + 1)}${_t.time(data1.getDate())}`;
              _t.formIframeSrc = _t.$apiForms +'/tbIetp-reportForm/ReportEmitter?rpt=/4103车站进出站量分时段统计.brt&params=p_line_id='+ _t.formItem.lines.substr(0, 2) +';p_interval=' + _t.formItem.interval + ';p_start=' + stopData + ';&grabheight=true&fitwidth=true';
            }
          }
        });
      },
