    ...
    methods: {
      ...
      getData() {
        ...
        _t.$refs.formData.validate(val => {
          if (val) {
            _t.getHeight();
            if (_t.formItem.lines !== '' && _t.formItem.dateTime.length !== 0 && _t.formItem.type !== '') {
              // 嵌入网页是否显示
              _t.isShow = true;
              var data1 = new Date(_t.formItem.dateTime);
              var stopData = `${data1.getFullYear()}${_t.time(data1.getMonth() + 1)}${_t.time(data1.getDate())}`;
              _t.formIframeSrc = _t.$apiForms +'/tbIetp-reportForm/ReportEmitter?rpt=/5102设备完好率统计报表.brt&params=p_line_id='+ _t.formItem.lines.substr(0,2) + ';p_device_type='+_t.formItem.type+';p_start='+stopData+';&grabheight=true&fitwidth=true';
            }
          }
        });
      },
