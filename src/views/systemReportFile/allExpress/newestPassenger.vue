        ...
        _t.$refs.formData.validate(val => {
          if (val) {
            ...
            if (1) {
              ...
              var stopData = `${data1.getFullYear()}${_t.time(data1.getMonth() + 1)}${_t.time(data1.getDate())}${_t.time(data1.getHours())}${_t.time(data1.getMinutes())}`;
              var startData = `${data2.getFullYear()}${_t.time(data2.getMonth() + 1)}${_t.time(data2.getDate())}${_t.time(data2.getHours())}${_t.time(data2.getMinutes())}`;
              _t.formIframeSrc = _t.$apiForms +'/tbIetp-reportForm/ReportEmitter?rpt=/1102实时客流统计报表.brt&params=p_line_id=' +_t.formItem.lines.substr(0,2)+ ';p_interval=' + _t.formItem.interval + ';p_stop=' + startData + ';p_start=' + stopData + ';&grabheight=true&fitwidth=true';
            }
          }
        });
      },
