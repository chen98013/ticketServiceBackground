      ...
      _t.$refs.formData.validate((val) => {
        if (val) {
          ...
          if (1) {
            // 嵌入网页是否显示
            _t.isShow = true;
            _t.formIframeSrc = _t.$apiForms +'/tbIetp-reportForm/ReportEmitter?rpt=/线路单程票无效票回收统计表.brt&params=p_line_id=' +_t.formItem.lines.substr(0,2)+ ';p_date=' + _t.formItem.dateTime  + ';&grabheight=true&fitwidth=true';
          }
        }
      });
    },
