      ...
      _t.$refs.formData.validate((val) => {
        if (val) {
          ...
          if (1) {
            // 嵌入网页是否显示
            _t.isShow = true;
            _t.formIframeSrc = _t.$apiForms +'/tbIetp-reportForm/ReportEmitter?rpt=/票务TVM票款差异结算报表.brt&params=p_station_id=' + _t.formItem.station + ';p_date=' + _t.formItem.dateTime  + ';&grabheight=true&fitwidth=true';
          }
        }
      });
    },
