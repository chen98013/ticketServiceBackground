  ...
  <div>
    <div id="div" class="clearFix formBox-top">
      ...
      <el-form :model="formItem" ref="formData" :rules="rules2" label-width="120px" :inline="true">
        ...
        <el-form-item label="车票类型：" prop="ticketType" class="marBottom7">
          <el-select
            clearable
            class="width200"
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules2: {
          ticketType: [{type:'number', required: true, message: '不能为空', trigger:'change'}],
    ...
    methods: {
      ...
      getData() {
        ...
        _t.$refs.formData.validate(val => {
          if (val) {
            ...
            if (_t.formItem.ticketType !== '' && _t.formItem.lines !== '' && time) {
              // 嵌入网页是否显示
              _t.isShow = true;
              ...
              _t.formIframeSrc = _t.$apiForms +'/tbIetp-reportForm/ReportEmitter?rpt=/3102线路车票销售统计报表.brt&params=p_line_id='+ _t.formItem.lines.substr(0,2)+';p_ticket_type='+_t.formItem.ticketType+';p_start=' + stopData + ';p_stop=' + startData + ';&grabheight=true&fitwidth=true';
            }
          }
        });
      },
      ...
      getTicketType() {
        ...
        if (_t.cookieMap.location_number !== null
          ...
          _t.$api.post('/tsGateWay/baseService/queryCoreTicketType', params, function (res) {

            if (res.rtCode && res.rtCode === '14021') {
              _t.ticketType = res.rtData === null ? [] : res.rtData;
              // _t.ticketType.forEach(item => {
              //   item.TICKET_TYPE = Number(item.TICKET_TYPE);
              // })
            }
          });
        }
