        ...
        _t.$refs.formData.validate(val => {
          if (val) {
            ...
            if (_t.formItem.lines !== '' && time) {
              ...
              _t.formIframeSrc = _t.$apiForms +'/tbIetp-reportForm/ReportEmitter?rpt=/2105线路运营发售收益报表.brt&params=p_line_id='+_t.formItem.lines.substr(0,2)+';p_stop=' + startData + ';p_start=' + stopData + ';&grabheight=true&fitwidth=true';
            }
          }
        });
        if (_t.formItem.lines !== '' && _t.formItem.dateTime.length !== 0) {
          // 嵌入网页是否显示
          _t.isShow = true;
          var data1 = new Date(_t.formItem.dateTime[0]);
          var data2 = new Date(_t.formItem.dateTime[1]);
